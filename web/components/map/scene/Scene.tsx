...
import { useStore, type Draft, type Layers, type Watch } from "@/state/store";
import {
  bubbleFragment,
  bubbleVertex,
  hostFragment,
...
function SkyShell({
    ...
    () =>
      new THREE.ShaderMaterial({
        ...
        uniforms: {
          ...
          uHeat: { value: 0 },
          uTonight: { value: 0 },
          cDeep: { value: tokenColor("--bg-deep", "#050506") },
  ...
  useFrame(({ clock }) => {
    // Bake the illustrated sky progressively, one cube face per frame, after the plain sky is up.
    if (material.uniforms.uBaked.value < 1) {
...
/** Bright catalogue stars (Yale BSC) on the sky sphere, for orientation. */
function CatalogStars({ data, visible }: { data: MapData; visible: boolean }) {
  ...
  const material = useMemo(
    () =>
      new THREE.ShaderMaterial({
        ...
        uniforms: { uPx: { value: 1 }, uFovScale: { value: 1 } },
      }),
    [],
  );
  useFrame(() => {
    ...
    material.uniforms.uFovScale.value = Math.min(2.2, Math.sqrt(BASE_FOV / camera.fov));
  });
...
function Hosts({
  ...
  const material = useMemo(
    () =>
      new THREE.ShaderMaterial({
        vertexShader: hostVertex,
        fragmentShader: hostFragment,
        transparent: true,
        depthWrite: false,
        uniforms: { ...watchUniforms, uPx: { value: 1 }, cAccent: { value: tokenColor("--accent", "#a3b8ff") } },
      }),
    [watchUniforms],
  );
  material.uniforms.uPx.value = dpr;
      ...
      new THREE.ShaderMaterial({
        ...
        fragmentShader: /* glsl */ `
          ...
          void main() {
            ...
            gl_FragColor = vec4(body + rim, uOpacity);
          }
        `,
      }),
    [],
...
/**
 * All pointer input on the canvas: draw a watch (draw mode), pick a star (look mode), pinch and
 * wheel zoom (both modes), and the pointer's RA/Dec for the status bar.
 */
function Input({
  data,
  index,
  ...
  }, [state.mode, state.layers.art, size]);
  ...
  useEffect(() => {
    ...
    const updateFovReadout = () => {
      if (hud.fov) hud.fov.textContent = `${camera.fov < 10 ? camera.fov.toFixed(1) : Math.round(camera.fov)}°`;
    };
    ...
    const zoomBy = (f: number) => {
      ...
      let fov = camera.fov;
      let d = dist;
      const maxD = controls.current?.maxDistance ?? 320;
      if (f < 1) {
        if (dist > MIN_DIST * 1.001) d = Math.max(MIN_DIST, dist * f);
        else fov = Math.max(MIN_FOV, fov * f);
      } else {
        if (fov < BASE_FOV * 0.999) fov = Math.min(BASE_FOV, fov * f);
        else d = Math.min(maxD, dist * f);
      }
      camera.position.setLength(d);
      camera.fov = fov;
      camera.updateProjectionMatrix();
      if (controls.current) controls.current.rotateSpeed = 0.5 * (fov / BASE_FOV);
      updateFovReadout();
      invalidate();
      scheduleDetail();
    };
    ...
    view.jumpTo = (ra, dec, fovDeg) => {
      ...
      // Stand just behind Earth and look through it at the target: the view from Earth.
      camera.position.set(-x * MIN_DIST, -y * MIN_DIST, -z * MIN_DIST);
      camera.fov = Math.max(MIN_FOV, Math.min(BASE_FOV, fovDeg));
      camera.updateProjectionMatrix();
      camera.lookAt(0, 0, 0);
      controls.current?.target.set(0, 0, 0);
      controls.current?.update();
      if (controls.current) controls.current.rotateSpeed = 0.5 * (camera.fov / BASE_FOV);
      updateFovReadout();
      invalidate();
      scheduleDetail();
    };
    ...
    const pickStar = (clientX: number, clientY: number) => {
      ...
      dispatch({ type: "selectStar", index: best >= 0 ? best : null });
    };
    ...
    const move = (e: PointerEvent) => {
      ...
      if (e.pointerType === "mouse" && hud.pointer) {
    ...
    const up = (e: PointerEvent) => {
      ...
      if (tap && Math.hypot(e.clientX - tap.x, e.clientY - tap.y) < 6) pickStar(e.clientX, e.clientY);
      tap = null;
    };
    ...
    const onControlsEnd = () => scheduleDetail();
    const ctl = controls.current;
    ctl?.addEventListener("end", onControlsEnd);
    ...
    return () => {
      ...
      clearTimeout(detailTimer);
      view.jumpTo = null;
      ctl?.removeEventListener("end", onControlsEnd);
  ...
  }, [gl, camera, dispatch, index, data.footprint, positionsRef, controls, baker, invalidate]);
...
export default function Scene({ data, index, showFps, onHeatMax }: SceneProps) {
  ...
  const drift = useMemo(() => !phone && !reducedMotion(), [phone]) && state.layers.art;
  ...
  return (
    <Canvas
      ...
      frameloop={showFps || drift ? "always" : "demand"}
      camera={{ fov: BASE_FOV, near: 0.05, far: 5000, position: [84, 30, 118] }}
      gl={{ antialias: !phone, powerPreference: "high-performance" }}
      onCreated={({ gl }) => gl.setClearColor(tokenColor("--bg-deep", "#050506"))}
      aria-hidden="true"
    >
      <SceneContent data={data} index={index} onHeatMax={onHeatMax} positionsRef={positionsRef} baker={baker} drift={drift} />
      {showFps && <FpsMeter />}
    </Canvas>
  );
}
  ...
  return (
    <>
      <OrbitControls
        ...
        enablePan={false}
        enableZoom={false}
        enableRotate={state.mode === "look"}
        rotateSpeed={0.5}
        minDistance={0.3}
        maxDistance={state.trueScale ? 700 : 320}
      />
      <SkyShell
        data={data}
        layers={state.layers}
      ...
      <Hosts index={index} trueScale={state.trueScale} watchUniforms={watchUniforms} positionsRef={positionsRef} />
      <Earth />
      {state.watches.map((w) => (
      ...
      <Input data={data} index={index} positionsRef={positionsRef} controls={controls} baker={baker} />
      <ReadoutAnchor />
