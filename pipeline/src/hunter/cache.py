...
def cache_dir() -> Path:
    root = os.environ.get("HUNTER_CACHE_DIR") or Path.home() / ".cache" / "planet-hunter" / "pipeline"
    path = Path(root)
...
def retry(fn: Callable[[], T], tries: int = 3, backoff_s: float = 2.0) -> T:
    """Call fn, retrying on any exception with exponential backoff (MAST and friends are flaky)."""
