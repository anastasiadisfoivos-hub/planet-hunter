...
KIND_ORDER = ["sky_context", "solar", "forecast_map", "cutout_reference", "cutout_new", "cutout_difference",
              "light_curve"]
...
def validate(images: list[Image], stats: Stats | None = None) -> list[Image]:
    ...
    A thumbnail that fails its own check falls back to the full image URL."""
    kept: list[Image] = []
    for img in images:
        ...
        if thumb and thumb != img["url"] and not get_net().probe(thumb).ok:
            log.info("thumbnail %s failed; using the full image", thumb)
            thumb = img["url"]
        kept.append({**img, "thumb_url": thumb, "width": p.width, "height": p.height})
    return kept
...
def order(images: list[Image]) -> list[Image]:
    ...
    for img in sorted(images, key=lambda i: rank.get(i["kind"], len(rank))):  # stable within a kind
        if img["url"] not in seen:
            seen.add(img["url"])
            out.append({"thumb_url": None, **img})  # images from elsewhere may predate thumb_url
    return out
